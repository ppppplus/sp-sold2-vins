...
def plot_junctions(input_image, junctions, junc_size=3, color=None):
    ...
    elif image.dtype in [np.float32, np.float64, np.float64]  and image.max() <= 2.:
        image = (image * 255.).astype(np.uint8)
    # A float type image ranging from 0.~255.
    elif image.dtype in [np.float32, np.float64, np.float64] and image.mean() > 10.:
