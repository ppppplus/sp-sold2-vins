...
class WunschLineMatcher(object):
    ...
    def forward(self, line_seg1, line_seg2, desc1, desc2):
        ...
        if self.sampling_mode == "regular":
            line_points1, valid_points1 = self.sample_line_points(line_seg1)    # linepoints为num_lines*num_samples*2的数组
            line_points2, valid_points2 = self.sample_line_points(line_seg2)    # valid_points对应有效点数的num_lines*num_samples的bool数组，False的点表明其没有从线段提取到
        else:
        ...
        line_points1 = torch.tensor(line_points1.reshape(-1, 2),
                                    dtype=torch.float, device=device)   # 做此转换后line_points将变为一个二维点集
        ...
        grid2 = keypoints_to_grid(line_points2, img_size2)  # grid是一个采样矩阵，用于定位desc对应的原图point位置
        desc1 = F.normalize(F.grid_sample(desc1, grid1)[0, :, :, 0], dim=0)
        desc2 = F.normalize(F.grid_sample(desc2, grid2)[0, :, :, 0], dim=0) # 得到的desc为128*num_line_points，对应每一个line_point的描述子
        ...
        # scores.shape = (n_lines1, n_lines2, num_samples, num_samples)，也就是每一对线段之间的匹配矩阵
