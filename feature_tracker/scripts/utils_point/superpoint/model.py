...
class SuperPointFrontend(object):
  ...
  def run(self, img):
    ...
    print("semi shape:{}, coarse_desc shape:{}".format(semi.shape, coarse_desc.shape))
...
class NnmPointMatchModel(BaseMatchModel):
  ...
  def _init(self, params):
    self.params = params
    self.maxl = self.params["max_length"]
    self.nn_thresh = self.params["nn_thresh"]
    if self.maxl < 2:
...
class SuperpointPointExtractModel(BaseExtractModel):
  def _init(self, params):
    self.params = params
