                                          ...
                                          int flags)
{
    ...
    if (image.channels() != 1 || (flags & cv::CALIB_CB_NORMALIZE_IMAGE))
    {
        ...
        if (image.channels() != 1)
        {
            cv::cvtColor(image, norm_img, cv::COLOR_GRAY2BGR);
            img = norm_img;
        }
                          ...
                          int dilation, bool firstRun)
{
    ...
    cv::findContours(image, contours, hierarchy, cv::RetrievalModes::RETR_CCOMP, cv::ContourApproximationModes::CHAIN_APPROX_SIMPLE);
...
Chessboard::checkChessboard(const cv::Mat& image, cv::Size patternSize) const
{
    ...
    for (float threshLevel = blackLevel; threshLevel < whiteLevel && !result; threshLevel += 20.0f)
    {
        ...
        // Initialize contour retrieving routine
        cv::findContours(thresh, contours, hierarchy, cv::RetrievalModes::RETR_CCOMP, cv::ContourApproximationModes::CHAIN_APPROX_SIMPLE);
